 ...
 *
 * @param {String} path
 * @param {SchemaTypeOptions} [options] See [SchemaTypeOptions docs](https:
 ...
 *     schema.path('name').validators.length;
 *
 * @property validators
 * @api public
 * @memberOf SchemaType
 ...
 * immutable paths unless the parent document has [`isNew: true`](https:
 *
 * #### Example:
 *
 ...
 * and `updateMany()` based on [strict mode](https:
 *
 * #### Example:
 *
 *
 ...
 *
 *
 *     const user = await User.find({ email: 'AVENUE@Q.COM' });
 *
 * As you can see above, setters allow you to transform the data before it
 * stored in MongoDB, or before executing a query.
 *
 * _NOTE: we could have also just used the built-in [`lowercase: true` SchemaType option](https:
 *
 ...
 * validation failed.
 *
 * The error message argument is optional. If not passed, the [default generic error message template](https:
 *
 * #### Example:
 *
 *
 ...
 *
 * Validation occurs `pre('save')` or whenever you manually execute [document#validate](https:
 *
 * If validation fails during `pre('save')` and no callback was passed to receive the error, an `error` event will be emitted on your Models associated db [connection](https:
 *
 ...
 * @param {Function} [obj.validator] validator function. If the validator function returns `undefined` or a truthy value, validation succeeds. If it returns [falsy](https:
 * @param {String|Function} [obj.message] optional error message. If function, should return the error message as a string
 * @param {Boolean} [obj.propsParameter=false] If true, Mongoose will pass the validator properties object (with the `validator` function, `message`, etc.) as the 2nd arg to the validator function. This is disabled by default because many validators [rely on positional args](https:
 ...
 * @return {SchemaType} this
 * @see Customized Error Messages https:
 * @see SchemaArray#checkRequired https:
 * @see SchemaBoolean#checkRequired https:
 * @see SchemaBuffer#checkRequired https:
 * @see SchemaNumber#checkRequired https:
 * @see SchemaObjectId#checkRequired https:
 * @see SchemaString#checkRequired https:
 * @api public
 */
...
/**
 * Set the model that this path refers to. This is the option that [populate](https:
 ...
 *     });
 *
 * @param {String|Model|Function} ref either a model name, a [Model](https:
 * @return {SchemaType} this
 * @api public
 */
 ...
 *     schema.path('name').getEmbeddedSchemaType();
 *     schema.path('tags').getEmbeddedSchemaType();
 *
