 ...
 * An instance of a Model is called a [Document](https:
 *
 * In Mongoose, the term "Model" refers to subclasses of the `mongoose.Model`
 * class. You should not use the `mongoose.Model` class directly. The
 * [`mongoose.model()`](https:
 * [`connection.model()`](https:
 ...
 * @param {Object} [fields] optional object containing the fields that were selected in the query which returned this document. You do **not** need to set this parameter to ensure Mongoose handles your [query projection](https:
 * @param {Boolean} [skipId=false] optional boolean. If true, mongoose doesn't add an `_id` field to the document.
 * @inherits Document https:
 ...
 *
 *     const conn2 = await mongoose.createConnection('mongodb:
 *     UserModel.useConnection(conn2);
 *
 *     UserModel.connection === mongoose.connection;
 *     UserModel.connection === conn2;
 *
 *     conn2.model('User') === UserModel;
 *     mongoose.model('User');
 *
 * Note: `useConnection()` does **not** apply any [connection-level plugins](https:
 ...
 * A Mongoose collection is a thin wrapper around a [MongoDB Node.js driver collection]([MongoDB Node.js driver collection](https:
...
/**
 * Saves this document by inserting a new document into the database if [document.isNew](https:
 * or sends an [updateOne](https:
 *
 * #### Example:
 *
 ...
 * @param {Object} [options.safe] (DEPRECATED) overrides [schema's safe option](https:
 ...
 * @param {Boolean} [options.timestamps=true] if `false` and [timestamps](https:
 * @param {Array} [options.pathsToSave] An array of paths that tell mongoose to only validate and save the paths in `pathsToSave`.
 * @throws {DocumentNotFoundError} if this [save updates an existing document](https:
 * @return {Promise}
 * @api public
 * @see middleware https:
 */
 ...
 *
 * Since `deleteOne()` returns a Query, the `deleteOne()` will **not** execute unless you use either `await`, `.then()`, `.catch()`, or [`.exec()`](https:
 *
 * #### Example:
 *
 *     product.deleteOne();
 ...
 *
 *     await Character.exists({ name: /picard/i });
 *     await Character.exists({ name: /riker/i });
 *
 ...
 *
 * @param {Object} filter
 * @param {Object} [options] optional see [`Query.prototype.setOptions()`](https:
 * @return {Query}
 */
 ...
 *
 * - `createCollection()` unless [`autoCreate`](https:
 * - `ensureIndexes()` unless [`autoIndex`](https:
 ...
 *
 * @api public
 * @param {Object} [options] see [MongoDB driver docs](https:
 * @returns {Promise}
 */
 ...
 *
 * See the [introductory blog post](https:
 ...
 *     await Customer.collection.createIndex({ age: 1 });
 *
...
/**
 * Create an [Atlas search index](https:
...
/**
 * Update an existing [Atlas search index](https:
 * This function only works when connected to MongoDB Atlas.
 *
 * #### Example:
 *
...
/**
 * Delete an existing [Atlas search index](https:
 * This function only works when connected to MongoDB Atlas.
 *
 * #### Example:
 *
...
/**
 * List all [Atlas search indexes](https:
 * This function only works when connected to MongoDB Atlas.
 *
 * #### Example:
 *
 ...
 *
 *     await Customer.createSearchIndex({ name: 'test', definition: { mappings: { dynamic: true } } });
 *     const res = await Customer.listSearchIndexes();
 *
 ...
 * use the [`autoIndex` option](https:
...
/**
 * Similar to `ensureIndexes()`, except for it uses the [`createIndex`](https:
 * function.
 *
 * @param {Object} [options] internal options
 * @return {Promise}
...
/**
 * Creates all [Atlas search indexes](https:
 * This function only works when connected to MongoDB Atlas.
 *
 * #### Example:
 *
 ...
 * See our [query casting tutorial](https:
 ...
 * @param {Object|String|String[]} [projection] optional fields to return, see [`Query.prototype.select()`](https:
 * @param {Object} [options] optional see [`Query.prototype.setOptions()`](https:
 * @param {Boolean} [options.translateAliases=null] If set to `true`, translates any schema-defined aliases in `filter`, `projection`, `update`, and `distinct`. Throws an error if there are any conflicts where both alias and raw property are defined on the same object.
 * @return {Query}
 * @see field selection https:
 * @see query casting https:
 * @api public
 */
 ...
 * @return {Query}
 * @see field selection https:
 * @see lean queries https:
 * @see findById in Mongoose https:
 * @api public
 */
 ...
 * use the [`estimatedDocumentCount()` function](https:
 * instead. If you call `countDocuments({})`, MongoDB will always execute
 * a full collection scan and **not** use any indexes.
 *
 * The `countDocuments()` function is similar to `count()`, but there are a
 * [few operators that `countDocuments()` does not support](https:
 ...
 * #### Example:
 *
 *     A.findOneAndUpdate(filter, update, options);
 *     A.findOneAndUpdate(filter, update);
 *     A.findOneAndUpdate(filter);
 *     A.findOneAndUpdate();
 *
 *
 *
 ...
 * @param {Object} [options.lean] if truthy, mongoose will return the document as a plain JavaScript object rather than a mongoose document. See [`Query.lean()`](https:
 * @param {ClientSession} [options.session=null] The session associated with this query. See [transactions docs](https:
 * @param {Boolean|String} [options.strict] overwrites the schema's [strict mode option](https:
 * @param {Boolean} [options.timestamps=null] If set to `false` and [schema-level timestamps](https:
 * @param {Boolean} [options.upsert=false] if true, and no documents found, insert a new document
 * @param {Object|String|String[]} [options.projection=null] optional fields to return, see [`Query.prototype.select()`](https:
 ...
 * @param {Boolean} [options.runValidators] if true, runs [update validators](https:
 * @param {Boolean} [options.setDefaultsOnInsert=true] If `setDefaultsOnInsert` and `upsert` are true, mongoose will apply the [defaults](https:
 * @param {Boolean} [options.includeResultMetadata] if true, returns the [raw result from the MongoDB driver](https:
 * @param {Boolean} [options.translateAliases=null] If set to `true`, translates any schema-defined aliases in `filter`, `projection`, `update`, and `distinct`. Throws an error if there are any conflicts where both alias and raw property are defined on the same object.
 ...
 * @param {Boolean} [options.runValidators] if true, runs [update validators](https:
 * @param {Boolean} [options.setDefaultsOnInsert=true] If `setDefaultsOnInsert` and `upsert` are true, mongoose will apply the [defaults](https:
 * @param {Boolean} [options.includeResultMetadata] if true, returns the full [ModifyResult from the MongoDB driver](https:
 * @param {Boolean} [options.upsert=false] if true, and no documents found, insert a new document
 * @param {Boolean} [options.new=false] if true, return the modified document rather than the original
 ...
 * @param {Boolean} [options.overwriteDiscriminatorKey=false] Mongoose removes discriminator key updates from `update` by default, set `overwriteDiscriminatorKey` to `true` to allow updating the discriminator key
 * @return {Query}
 * @see Model.findOneAndUpdate https:
 * @see mongodb https:
 * @api public
 */
 ...
 * @param {Boolean} [options.translateAliases=null] If set to `true`, translates any schema-defined aliases in `filter`, `projection`, `update`, and `distinct`. Throws an error if there are any conflicts where both alias and raw property are defined on the same object.
 * @return {Query}
 * @see Model.findOneAndDelete https:
 * @see mongodb https:
 */
 ...
 * #### Example:
 *
 *     A.findOneAndReplace(filter, replacement, options)
 ...
 * @param {Object} [options] Options passed down to `save()`. To specify `options`, `docs` **must** be an array, not a spread. See [Model.save](https:
 ...
 * - 'error': An unrecoverable error occurred. In particular, change streams currently error out if they lose connection to the replica set primary. Follow [this GitHub issue](https:
 ...
 * @param {Object} [options] see the [mongodb driver options](https:
 ...
 * @param {Boolean} [options.rawResult=false] if false, the returned promise resolves to the documents that passed mongoose document validation. If `true`, will return the [raw result from the MongoDB driver](https:
 * @param {Boolean} [options.lean=false] if `true`, skips hydrating the documents. This means Mongoose will **not** cast, validate, or apply defaults to any of the documents passed to `insertMany()`. This option is useful if you need the extra performance, but comes with data integrity risk. Consider using with [`castObject()`](https:
 ...
 * The only exception is [setting the `update` operator for `updateOne` or `updateMany` to a pipeline](https:
 *
 * This function does **not** trigger any middleware, neither `save()`, nor `update()`.
 * If you need to trigger
 * `save()` middleware for every document use [`create()`](https:
 *
 * #### Example:
 *
 *     Character.bulkWrite([
 ...
 * @param {Object} [ops.insertOne.timestamps=true] If false, do not apply [timestamps](https:
 ...
 * @param {Boolean} [ops.updateOne.timestamps=true] If false, do not apply [timestamps](https:
 * @param {Object} [ops.updateOne.collation] The [MongoDB collation](https:
 * @param {Array} [ops.updateOne.arrayFilters] The [array filters](https:
 ...
 * @param {Boolean} [ops.updateMany.timestamps=true] If false, do not apply [timestamps](https:
 * @param {Object} [ops.updateMany.collation] The [MongoDB collation](https:
 * @param {Array} [ops.updateMany.arrayFilters] The [array filters](https:
 ...
 * @param {Object} [ops.replaceOne.timestamps=true] If false, do not apply [timestamps](https:
 * @param {Object} [options]
 * @param {Boolean} [options.ordered=true] If true, execute writes in order and stop at the first error. If false, execute writes in parallel and continue until all writes have either succeeded or errored.
 * @param {Boolean} [options.timestamps=true] If false, do not apply [timestamps](https:
 * @param {ClientSession} [options.session=null] The session associated with this bulk write. See [transactions docs](https:
 ...
 * @param {Boolean|"throw"} [options.strict=null] Overwrites the [`strict` option](https:
 * @return {Promise} resolves to a [`BulkWriteOpResult`](https:
 * @api public
 */
 ...
 *     obj.num;
 *
 *     Test.castObject({ num: 'not a number' });
 *
 ...
 *
 *
 *     await Person.find({ name: /Stark$/ }).updateMany({ isDeleted: true });
 *     await Person.find().updateMany({ isDeleted: true });
 *
 * This function triggers the following middleware.
 *
 * - `updateMany()`
 ...
 * @param {Boolean} [options.overwriteDiscriminatorKey=false] Mongoose removes discriminator key updates from `update` by default, set `overwriteDiscriminatorKey` to `true` to allow updating the discriminator key
 * @return {Query}
 * @see Query docs https:
 * @see MongoDB docs https:
 * @see UpdateResult https:
 * @api public
 */
 ...
 *
 *
 *     await Person.findOne({ name: 'Jean-Luc Picard' }).updateOne({ ship: 'USS Enterprise' });
 *     await Person.updateOne({ ship: 'USS Enterprise' });
 *
 * This function triggers the following middleware.
 *
 * - `updateOne()`
 ...
 * #### Note:
 *
 * - Mongoose does **not** cast aggregation pipelines to the model's schema because `$project` and `$group` operators allow redefining the "shape" of the documents at any stage of the pipeline, which may leave documents in an incompatible format. You can use the [mongoose-cast-aggregation plugin](https:
 * - The documents returned are plain javascript objects, not mongoose documents (since any shape of document can be returned).
 *
 * #### More About Aggregations:
 *
 * - [Mongoose `Aggregate`](https:
 * - [An Introduction to Mongoose Aggregate](https:
 ...
 * @param {string|PopulateOptions} [options.populate=null] Recursively populate paths in the populated documents. See [deep populate docs](https:
 * @param {boolean} [options.retainNullValues=false] By default, Mongoose removes null and undefined values from populated arrays. Use this option to make `populate()` retain `null` and `undefined` array entries.
 * @param {boolean} [options.getters=false] If true, Mongoose will call any getters defined on the `localField`. By default, Mongoose gets the raw value of `localField`. For example, you would need to set this option to `true` if you wanted to [add a `lowercase` getter to your `localField`](https:
